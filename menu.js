// menu.js - UPDATED VERSION WITH PAYSTACK PAYMENT INTEGRATION
// ========= GOLDEN STAR RESTAURANT MENU =========

// CONFIGURATION
const CONFIG = {
    restaurantName: "Golden Star Restaurant",
    whatsappTo: "+254740940395",
    mpesaTill: "888555",
    bankName: "KCB Bank",
    bankAccount: "0123456789",
    bankAccountName: "GOLDEN STAR RESTAURANT",
    // PAYSTACK CONFIGURATION
    paystackPublicKey: "pk_live_1aede7667d50adb2147614cb580a9513552c0aa9",
    paystackSecretKey: "sk_live_67c2f9d5291333d490e10aacb91bfbfa7588851e",
    paystackCallbackUrl: "https://goldenstar.com/callback",
    paystackWebhookUrl: "https://goldenstar.com/webhook"
};
...
function refreshAmountsInPaymentPanels() {
    ...
    // Update Paystack amount in the M-Pesa section
    const paystackAmountInMpesa = el('paystackAmountInMpesa');
    if (paystackAmountInMpesa) {
        paystackAmountInMpesa.textContent = fmtKES(total);
    }
    
    // Update Paystack payment button
    const paystackPayBtn = el('paystackPayBtn');
    if (paystackPayBtn) {
        paystackPayBtn.textContent = `Pay ${fmtKES(total)} via Paystack`;
    }
}

// Paystack Payment Integration
function initializePaystackPayment() {
    const totalAmount = getCartTotal() * 100; // Convert to kobo/pesewas
    const email = el('customerEmail')?.value || 'customer@goldenstar.com';
    const phone = el('customerPhone')?.value || '';
    const table = getTableLabel();
    
    if (!email) {
        showToast("Please enter your email address for payment receipt", "warning");
    ...
    const reference = `GS${Date.now()}${Math.floor(Math.random() * 1000)}`;
    ...
    const handler = PaystackPop.setup({
        ...
        metadata: {
            custom_fields: [
                {
                    display_name: "Table/Service",
                    variable_name: "table",
                    value: table || "Take Away"
                },
                {
        ...
        callback: function(response) {
            ...
            // Finalize order with Paystack payment method
            finalizeOrderWithPayment("M-Pesa via Paystack", reference);
...
// Order Finalization WITHOUT WhatsApp forwarding
function finalizeOrderWithPayment(paymentLabel, paymentReference = "") {
    ...
    // NO WHATSAPP FORWARDING - Direct to success screen
    setTimeout(() => {
...
function init() {
    console.log('Initializing Golden Star Restaurant Menu System with Paystack...');
    ...
    console.log('System initialized successfully with 64 menu items and Paystack integration');
}
...
function setupPaymentEventListeners() {
    ...
    el('checkoutBtn')?.addEventListener('click', function() {
        ...
        // Select M-Pesa by default (which now includes Paystack)
        setTimeout(() => {
            document.querySelectorAll(".payment-option").forEach(x => x.classList.remove("selected"));
            const mpesaOption = document.querySelector('.payment-option[data-method="mpesa"]');
            if (mpesaOption) {
                mpesaOption.classList.add("selected");
                window.selectedPayment = "mpesa";
                
                // Show M-Pesa panel
                ["mpesa","bank","cash"].forEach(m => {
                    const detailsEl = el(m + "Details");
                    if (detailsEl) detailsEl.classList.add("hidden");
                });
                const mpesaDetails = el("mpesaDetails");
                if (mpesaDetails) mpesaDetails.classList.remove("hidden");
            }
        }, 100);
    });
    ...
    // Paystack payment button (in M-Pesa section)
    el('paystackPayBtn')?.addEventListener('click', function() {
    ...
    // Payment confirmation buttons (traditional methods)
    el('confirmMpesa')?.addEventListener('click', () => finalizeOrderWithPayment("M-Pesa Till"));
    el('confirmBank')?.addEventListener('click', () => finalizeOrderWithPayment("Bank Transfer"));
    el('confirmCash')?.addEventListener('click', () => finalizeOrderWithPayment("Cash"));
    ...
    el('sendWhatsApp')?.addEventListener('click', function() {
        ...
        const items = window.lastOrderPayload.items || [];
        ...
        const customerMsg = [
            ...
            ``,
            `📋 *Order ID:* ${window.lastOrderPayload.orderId}`,
            `🏠 *Table:* ${window.lastOrderPayload.table}`,
            `💰 *Payment:* ${window.lastOrderPayload.paymentLabel}`,
            window.lastOrderPayload.paymentReference ? `🔢 *Payment Ref:* ${window.lastOrderPayload.paymentReference}` : '',
            `💵 *Total:* ${fmtKES(window.lastOrderPayload.total)}`,
            ``,
            `📝 *Your Order:*`,
            ...lines,
            ``,
    ...
    // Customer email/phone input validation
    el('customerEmail')?.addEventListener('input', function() {
        const btn = el('paystackPayBtn');
        if (btn && this.value) {
            btn.disabled = false;
        }
    });
